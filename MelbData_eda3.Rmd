...
p1 <- train %>%
  ...
  mutate(wday = wday(DateTime.x, label = TRUE)) %>%

  group_by(wday, Card_SubType_Desc.x.x) %>%

  summarise(median_duration = median(diff_hours)/60) %>%

  ggplot(aes(wday, median_duration, color = Card_SubType_Desc.x.x)) +
...
p2 <- train %>%
  ...
  mutate(hpick = hour(DateTime.y)) %>%

  group_by(hpick, Card_SubType_Desc.y.x) %>%

  summarise(median_duration = median(trip_duration)/60) %>%

  ggplot(aes(hpick, median_duration, color = Card_SubType_Desc.x.x)) +
...
train %>%
  ...
  ggplot(aes(VehicleID.x, diff_hours, color = VehicleID.x)) +
  ...
  labs(y = "Trip duration [s]", x = "Vehicle")
